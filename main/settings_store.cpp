...
Settings SettingsStore::load() const {
    ...
    auto load_service = [&](ServiceSettings& service, const char* prefix) {
        ...
        if(service.provider=="gemini"){
            service.url="https://generativelanguage.googleapis.com/v1beta";service.voice.clear();
            if(std::strcmp(prefix,"stt")==0)service.model="gemini-3.5-transcribe";
            else if(std::strcmp(prefix,"llm")==0)service.model="gemini-3.5-flash-lite";
            else service.model="gemini-3.1-flash-tts-preview";
...
bool SettingsStore::save(const Settings& value) const {
    ...
    auto save_service = [&](const ServiceSettings& service,const char* prefix){
        const std::string p(prefix);return put((p+"_prov").c_str(),service.provider)&&put((p+"_url").c_str(),service.url)&&put((p+"_model").c_str(),service.model)&&put((p+"_voice").c_str(),service.voice)&&put((p+"_key").c_str(),service.api_key);
    };
