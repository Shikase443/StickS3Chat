...
std::string limitVoiceText(const std::string& text){
    constexpr size_t MAX_CHARACTERS=250;size_t position=0,count=0;while(position<text.size()&&count<MAX_CHARACTERS){position+=std::min(utf8Length(static_cast<uint8_t>(text[position])),text.size()-position);++count;}if(position>=text.size())return text;size_t cut=position,best=0;for(const char* mark:{"。","！","？",". ","! ","? ","\n"}){size_t found=text.rfind(mark,position);if(found!=std::string::npos&&found>=position/2){size_t end=found+std::strlen(mark);if(end>best)best=end;}}bool sentence=best>0;if(sentence)cut=best;std::string out=text.substr(0,cut);while(!out.empty()&&(out.back()==' '||out.back()=='\r'||out.back()=='\n'||out.back()=='\t'))out.pop_back();if(!sentence)out+="…";return out;
}
...
bool VoiceChat::begin(){return xTaskCreatePinnedToCore(taskEntry,"voice_api",24576,this,4,&task_,1)==pdPASS;}
...
void VoiceChat::fail(const std::string& m){stopCaption();setState(VoiceState::ERROR,m);vTaskDelay(pdMS_TO_TICKS(1800));setState(VoiceState::IDLE);}
...
void VoiceChat::finishRecording(){M5.Mic.end();heap_caps_free(capture_);capture_=nullptr;if(recording_samples_<SAMPLE_RATE/4){setState(VoiceState::IDLE);heap_caps_free(recording_);recording_=nullptr;recording_samples_=0;return;}if(pending_settings_.connection_mode=="integrated")setState(VoiceState::LLM_PROCESSING,"Processing...");else setState(VoiceState::STT_PROCESSING,"Recognizing...");xTaskNotifyGive(task_);}
void VoiceChat::update(bool pressed,bool released,bool wifi,const Settings& settings){
    auto s=state();if(s==VoiceState::IDLE&&pressed){if(!wifi){setState(VoiceState::ERROR,"Wi-Fi disconnected");return;}pending_settings_=settings;if(!startRecording())setState(VoiceState::ERROR,"Recording failed");}
    ...
    else if(s==VoiceState::ERROR){static int64_t since=0;if(!since)since=esp_timer_get_time();if(esp_timer_get_time()-since>1800000){since=0;setState(VoiceState::IDLE);}}
}
void VoiceChat::taskEntry(void* arg){static_cast<VoiceChat*>(arg)->taskLoop();}
void VoiceChat::taskLoop(){for(;;){ulTaskNotifyTake(pdTRUE,portMAX_DELAY);Settings cfg=pending_settings_;std::unique_ptr<int16_t,PsramDeleter> pcm(recording_);size_t pcm_samples=recording_samples_;recording_=nullptr;recording_samples_=0;std::string user,answer;
    if(cfg.connection_mode=="integrated"){
        setState(VoiceState::LLM_PROCESSING,"Processing...");if(!integrated(cfg,pcm.get(),pcm_samples,user,answer)){if(cancel_requested_.exchange(false)){stopCaption();setState(VoiceState::IDLE);continue;}fail("Integrated API failed");continue;}pcm.reset();
    ...
    stack_low_water_.store(uxTaskGetStackHighWaterMark(nullptr));free_heap_.store(heap_caps_get_free_size(MALLOC_CAP_8BIT));ESP_LOGI("voice_chat","stack low water=%u, free heap=%u",static_cast<unsigned>(stack_low_water_.load()),static_cast<unsigned>(free_heap_.load()));}}
...
bool VoiceChat::integrated(const Settings& cfg,const int16_t* pcm,size_t pcm_samples,std::string& transcript,std::string& answer){
    const auto& s=cfg.integrated;if(s.url.empty())return false;
    ...
    esp_http_client_config_t config{};config.url=s.url.c_str();config.method=HTTP_METHOD_POST;config.timeout_ms=300000;config.crt_bundle_attach=esp_crt_bundle_attach;config.buffer_size=4096;config.buffer_size_tx=4096;auto client=esp_http_client_init(&config);if(!client)return false;std::string type="multipart/form-data; boundary="+std::string(boundary);esp_http_client_set_header(client,"Content-Type",type.c_str());std::string authorization;if(!s.api_key.empty()){authorization="Bearer "+s.api_key;esp_http_client_set_header(client,"Authorization",authorization.c_str());}
    size_t request_size=prefix.size()+header.size()+pcm_samples*2+suffix.size();bool ok=esp_http_client_open(client,request_size)==ESP_OK&&sendAll(client,prefix.data(),prefix.size())&&sendAll(client,header.data(),header.size())&&sendAll(client,pcm,pcm_samples*2)&&sendAll(client,suffix.data(),suffix.size());if(!ok||esp_http_client_fetch_headers(client)<0||esp_http_client_get_status_code(client)<200||esp_http_client_get_status_code(client)>=300){esp_http_client_cleanup(client);return false;}
    HttpReader reader(client);std::string line;size_t ignored=0;if(!reader.readLine(line)||line.rfind("--",0)!=0||!readPartHeaders(reader,ignored)){esp_http_client_cleanup(client);return false;}std::string metadata;bool found=false;while(reader.readLine(line)){if(line.rfind("--",0)==0){found=true;break;}if(metadata.size()+line.size()+1>MAX_JSON_RESPONSE){esp_http_client_cleanup(client);return false;}metadata+=line;metadata.push_back('\n');}auto* json=found?cJSON_ParseWithLength(metadata.data(),metadata.size()):nullptr;if(!json){esp_http_client_cleanup(client);return false;}transcript=jsonString(json,"transcript");answer=jsonString(json,"answer");auto session=jsonString(json,"sessionKey");cJSON_Delete(json);if(!session.empty())integrated_session_=session;
    size_t audio_remaining=0;if(!readPartHeaders(reader,audio_remaining)||!audio_remaining){esp_http_client_cleanup(client);return false;}startCaption(answer.empty()?transcript:answer);bool played=playStream(reader,audio_remaining,true);esp_http_client_cleanup(client);return played;
}
...
bool VoiceChat::complete(const Settings& cfg,const std::string& user,std::string& out){const auto& s=cfg.llm;if(s.url.empty()||s.model.empty())return false;auto* root=cJSON_CreateObject();cJSON_AddStringToObject(root,"model",s.model.c_str());
    ...
    else{auto* messages=cJSON_AddArrayToObject(root,"messages");if(!external_session)for(const auto& h:history_){auto* m=cJSON_CreateObject();cJSON_AddStringToObject(m,"role",h.role.c_str());cJSON_AddStringToObject(m,"content",h.text.c_str());cJSON_AddItemToArray(messages,m);}auto* m=cJSON_CreateObject();cJSON_AddStringToObject(m,"role","user");cJSON_AddStringToObject(m,"content",user.c_str());cJSON_AddItemToArray(messages,m);if(s.provider=="anthropic")cJSON_AddNumberToObject(root,"max_tokens",256);else if(endsWith(s.url,"/responses"))cJSON_AddNumberToObject(root,"max_output_tokens",256);else cJSON_AddNumberToObject(root,"max_tokens",256);if(endsWith(s.url,"/responses")){cJSON_DetachItemFromObject(root,"messages");cJSON_AddItemToObject(root,"input",messages);}}
...
bool VoiceChat::synthesizeAndPlay(const Settings& cfg,const std::string& text,std::string& error){
    ...
    auto* root=cJSON_CreateObject();if(!root)return false;cJSON_AddStringToObject(root,"model",s.model.c_str());cJSON_AddStringToObject(root,"input",text.c_str());cJSON_AddStringToObject(root,"voice",s.voice.empty()?"alloy":s.voice.c_str());cJSON_AddStringToObject(root,"response_format","wav");auto body=jsonPrint(root);cJSON_Delete(root);if(body.empty())return false;
...
bool VoiceChat::synthesizeGeminiAndPlay(const Settings& cfg,const std::string& text,std::string& error){
    ...
    auto* contents=cJSON_AddArrayToObject(root,"contents");auto* item=cJSON_CreateObject();cJSON_AddItemToArray(contents,item);auto* parts=cJSON_AddArrayToObject(item,"parts");auto* part=cJSON_CreateObject();cJSON_AddStringToObject(part,"text",text.c_str());cJSON_AddItemToArray(parts,part);
...
bool VoiceChat::playStream(HttpReader& reader,size_t bytes_remaining,bool bounded){
    ...
    uint32_t rate=0;uint16_t channels=0,bits=0;bool started=false,success=false;constexpr size_t BUFFER_SIZE=4096,BUFFER_COUNT=3;std::unique_ptr<uint8_t,ByteDeleter> buffers(static_cast<uint8_t*>(heap_caps_malloc(BUFFER_SIZE*BUFFER_COUNT,MALLOC_CAP_8BIT)));if(!buffers)return false;size_t buffer_index=0;
...
bool VoiceChat::synthesize(const Settings& cfg,const std::string& text,std::vector<uint8_t>& out,bool& raw,std::string& error){const auto& s=cfg.tts;if(s.url.empty()||s.model.empty()){error="TTS config error";return false;}auto* root=cJSON_CreateObject();if(s.provider!="gemini")cJSON_AddStringToObject(root,"model",s.model.c_str());
    if(s.provider=="gemini"){auto* contents=cJSON_AddArrayToObject(root,"contents");auto* item=cJSON_CreateObject();cJSON_AddItemToArray(contents,item);auto* parts=cJSON_AddArrayToObject(item,"parts");auto* p=cJSON_CreateObject();cJSON_AddStringToObject(p,"text",text.c_str());cJSON_AddItemToArray(parts,p);auto* gen=cJSON_AddObjectToObject(root,"generationConfig");auto* mods=cJSON_AddArrayToObject(gen,"responseModalities");cJSON_AddItemToArray(mods,cJSON_CreateString("AUDIO"));auto* speech=cJSON_AddObjectToObject(gen,"speechConfig");auto* voice=cJSON_AddObjectToObject(speech,"voiceConfig");auto* pre=cJSON_AddObjectToObject(voice,"prebuiltVoiceConfig");cJSON_AddStringToObject(pre,"voiceName",s.voice.empty()?"Kore":s.voice.c_str());}
    else{cJSON_AddStringToObject(root,"input",text.c_str());cJSON_AddStringToObject(root,"voice",s.voice.empty()?"alloy":s.voice.c_str());cJSON_AddStringToObject(root,"response_format","wav");}
